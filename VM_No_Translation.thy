...
definition
  hoare_pure :: "('regv event \<Rightarrow> bool) \<Rightarrow> bool \<Rightarrow> ('regv, 'a, 'e) monad \<Rightarrow>
    ('a \<Rightarrow> bool) \<Rightarrow> ('e \<Rightarrow> bool) \<Rightarrow> bool"
  where
  "hoare_pure assms P m Q E = (\<forall> t r. (m, t, r) \<in> Traces \<and> (\<forall>ev \<in> set t. assms ev) \<and> P \<longrightarrow>
    (case r of
...
lemma hoare_pure_RunD:
  "hoare_pure assms P m Q E \<Longrightarrow>
...
lemma hoare_pure_bind:
  "(\<forall>x. hoare_pure assms (Q x) (g x) R E) \<Longrightarrow>
    hoare_pure assms P f Q E \<Longrightarrow>
    hoare_pure assms P (Sail2_prompt_monad.bind f (\<lambda>x. g x)) R E"
  apply (clarsimp simp: hoare_pure_def)
  apply (erule bind_Traces_final_cases[THEN disjE])
   apply (simp split: monad.split)
   apply auto[1]
  apply (fastforce simp: ball_Un)
  done

lemma hoare_pure_meta:
  "hoare_pure assms P f Q E \<Longrightarrow>
    hoare_pure assms (P \<and> (\<forall>x. Q x \<longrightarrow> Q' x)) f Q' E"
  apply (clarsimp simp add: hoare_pure_def split: monad.split)
  apply fastforce
  done

lemma hoare_pure_triv1:
  "hoare_pure assms True f (\<lambda>_. True) (\<lambda>_. True)"
...
lemma hoare_pure_triv_no_exception1:
  "monad_no_exception S f \<Longrightarrow> hoare_pure assms (S \<subseteq> {e. E e}) f (\<lambda>_. True) E"
  apply (clarsimp simp: hoare_pure_def split: monad.split)
...
lemma hoare_pure_return:
  "hoare_pure assms (Q x) (return x) Q E"
  by (simp add: hoare_pure_def return_def)

lemma hoare_pure_weaken:
  "hoare_pure assms P f Q E \<Longrightarrow> (P' \<longrightarrow> P) \<Longrightarrow>
    hoare_pure assms P' f Q E"
  by (clarsimp simp add: hoare_pure_def)

lemma read_reg_hoare_pure1:
  "hoare_pure assms True (read_reg ref)
    (\<lambda>x. \<exists>regv. assms (E_read_reg (name ref) regv) \<and>
        of_regval ref regv = Some x) E"
...
lemma read_PSTATE_no_translation:
  "hoare_pure sctlr_no_translation (\<forall>ps. ProcState_EL ps = EL0 \<longrightarrow> Q ps)
    (read_reg PSTATE_ref) Q E"
...
lemma read_SCTLR_no_translation:
  "hoare_pure sctlr_no_translation (\<forall>sreg. \<not> lsb sreg \<longrightarrow> Q sreg)
    (read_reg SCTLR_EL1_ref) Q E"
  apply (rule hoare_pure_weaken, rule read_reg_hoare_pure)
  apply (clarsimp simp: PSTATE_ref_def SCTLR_EL1_ref_def)
  done
...
lemma hoare_pure_If:
  "hoare_pure assms P f Q E \<Longrightarrow>
    hoare_pure assms P' g Q E \<Longrightarrow>
    hoare_pure assms (if x then P else P') (if x then f else g) Q E"
  by simp

lemma hoare_pure_Fail:
  "hoare_pure assms True (exit0 ()) Q E"
  "hoare_pure assms True (Fail str) Q E"
  by (simp add: exit0_def hoare_pure_def)+

lemma hoare_pure_assert_exp:
  "hoare_pure assms (P \<longrightarrow> Q ()) (assert_exp P str) Q E"
...
lemma ELIsInHost_no_translation:
  "hoare_pure sctlr_no_translation True
    (ELIsInHost el) (\<lambda>rv. el = EL0 \<longrightarrow> rv = False) E"
...
lemma S1TranslationRegime__1_no_translation:
  "hoare_pure sctlr_no_translation True
    (S1TranslationRegime__1 ()) (\<lambda>el. el = EL1) E"
...
lemma SCTLR_read__1_no_translation:
  "hoare_pure sctlr_no_translation True
    (SCTLR_read__1 ()) (\<lambda>sreg. \<not> lsb sreg) E"
...
lemma hoare_pure_pair_case:
  "(\<And>x y. hoare_pure assms (P x y) (f x y) Q E) \<Longrightarrow>
    hoare_pure assms (case t of (x, y) \<Rightarrow> P x y) (case t of (x, y) \<Rightarrow> f x y) Q E"
  by (cases t; simp)

lemma hoare_pure_fetch_asm:
  "(P \<Longrightarrow> hoare_pure assms P' f Q E) \<Longrightarrow>
    hoare_pure assms (P \<and> P') f Q E"
  by (clarsimp simp add: hoare_pure_def)

lemma hoare_pure_pre_cont:
  "hoare_pure assms False f Q E"
  by (clarsimp simp add: hoare_pure_def)

lemma AArch64_AddressSizeFault_no_translation:
  "hoare_pure sctlr_no_translation True
    (AArch64_AddressSizeFault addr lvl acctype iswrite snd_stage s2fs)
    (\<lambda>desc. FaultRecord_statuscode desc \<noteq> Fault_None) E"
...
lemma AArch64_NoFault_no_translation:
  "hoare_pure sctlr_no_translation True
    (AArch64_NoFault uu)
    (\<lambda>desc. FaultRecord_statuscode desc = Fault_None) E"
...
lemma hoare_pure_throw:
  "hoare_pure assms (E ex) (throw ex) P E"
  by (simp add: hoare_pure_def throw_def)

lemma hoare_pure_try_catch:
  "(\<forall>x. hoare_pure assms (Q x) (g x) R E) \<Longrightarrow>
    hoare_pure assms P f R Q \<Longrightarrow>
    hoare_pure assms P (try_catch f (\<lambda>x. g x)) R E"
  apply (clarsimp simp: hoare_pure_def)
...
lemma hoare_pure_catch_early_return:
  "hoare_pure assms P f Q (case_sum Q E) \<Longrightarrow>
    hoare_pure assms P (catch_early_return f) Q E"
...
lemma hoare_pure_liftR:
  "hoare_pure assms P f Q (E o Inr) \<Longrightarrow>
    hoare_pure assms P (liftR f) Q E"
...
lemma AArch64_FirstStageTranslateWithTag_no_translation:
  ...
  apply (clarsimp simp: IsFault_def)
  done
