...
definition
  "is_load_auth_cap c s \<equiv>
     (is_original_load_auth_cap c s \<or>
      (\<exists>c'. is_original_load_auth_cap c' s \<and> is_indirect_sentry CC c' \<and> CapUnseal c' = c \<and>
            c \<in> invoked_indirect_caps \<and> \<not>is_fetch))"

lemma is_original_load_auth_cap_is_load_auth_cap:
  "is_original_load_auth_cap c s \<Longrightarrow> is_load_auth_cap c s"
  by (auto simp: is_load_auth_cap_def)

lemma is_original_load_auth_cap_run_imp[derivable_caps_runI]:
  "is_original_load_auth_cap c s \<Longrightarrow> is_original_load_auth_cap c (run s t)"
  using derivable_caps_run_imp[of c s t]
  by (auto simp: is_original_load_auth_cap_def)

lemma is_load_auth_cap_run_imp[derivable_caps_runI]:
  "is_load_auth_cap c s \<Longrightarrow> is_load_auth_cap c (run s t)"
  by (auto simp: is_load_auth_cap_def intro: is_original_load_auth_cap_run_imp)

lemmas read_reg_PCC_is_load_auth_cap[derivable_capsE] =
  read_reg_PCC_is_original_load_auth_cap[THEN is_original_load_auth_cap_is_load_auth_cap]
...
lemma is_load_auth_cap_load_caps_permitted_iff:
  assumes "is_load_auth_cap c s"
  shows "cap_permits CAP_PERM_LOAD_CAP c \<longleftrightarrow> load_caps_permitted"
  using assms
  unfolding is_load_auth_cap_def is_original_load_auth_cap_def
  by (auto simp: cap_permits_CapUnseal_iff)

lemma C_read_is_original_load_auth_cap:
  assumes "Run (C_read n) t c"
    and "{''_R29''} \<subseteq> accessible_regs s"
    and "load_cap_trace_assms t"
    and "CapIsTagSet c \<or> n \<noteq> 31"
    and "loads_via_cap_reg n"
  shows "is_original_load_auth_cap c (run s t)"
  using assms C_read_derivable[OF assms(1,2)]
  by (elim Run_C_readE) (auto simp add: is_original_load_auth_cap_def load_cap_trace_assms_def CapNull_def)

lemma C_read_is_original_load_auth_cap'[derivable_capsE]:
  assumes "Run (C_read n) t c"
    and "{''_R29''} \<subseteq> accessible_regs s"
    and "load_cap_trace_assms t"
    and "CapIsTagSet c \<or> n \<noteq> 31"
    and "load_auth = Some (RegAuth n) \<or> load_auth = None"
    and "\<not>is_fetch"
  shows "is_original_load_auth_cap c (run s t)"
  using assms
  by (elim C_read_is_original_load_auth_cap[unfolded loads_via_cap_reg_def cap_reg_is_load_auth_def]) auto

lemma CSP_read_is_original_load_auth_cap:
  assumes "Run (CSP_read u) t c"
    and "load_cap_trace_assms t"
    and "loads_via_cap_reg 31"
  shows "is_original_load_auth_cap c (run s t)"
proof -
  from assms(1) obtain r where "E_read_reg r (Regval_bitvector_129_dec c) \<in> set t" and r: "r \<in> R_name 31"
    by (elim Run_CSP_readE)
  with assms(2) have "load_cap_ev_assms (E_read_reg r (Regval_bitvector_129_dec c))"
    unfolding load_cap_trace_assms_def
    by blast
  then show ?thesis
    using assms(3) r CSP_read_derivable[OF assms(1), where s = s]
    unfolding is_original_load_auth_cap_def
    by auto
qed

lemma CSP_read_is_original_load_auth_cap'[derivable_capsE]:
  assumes "Run (CSP_read u) t c"
    and "load_cap_trace_assms t"
    and "load_auth = Some (RegAuth 31) \<or> load_auth = None"
    and "\<not>is_fetch"
  shows "is_original_load_auth_cap c (run s t)"
  using assms
  by (elim CSP_read_is_original_load_auth_cap[unfolded loads_via_cap_reg_def cap_reg_is_load_auth_def]) auto

lemma CSP_or_C_read_is_original_load_auth_cap:
  assumes "Run (if n = 31 then m \<then> CSP_read u else C_read n) t c"
    and "load_cap_trace_assms t"
    and "loads_via_cap_reg n"
    and "{''_R29''} \<subseteq> accessible_regs s"
  shows "is_original_load_auth_cap c (run s t)"
  using assms
  by (auto elim!: Run_bindE C_read_is_original_load_auth_cap CSP_read_is_original_load_auth_cap split: if_splits)

lemma CSP_or_C_read_is_original_load_auth_cap'[derivable_capsE]:
  assumes "Run (if n = 31 then m \<then> CSP_read u else C_read n) t c"
    and "load_cap_trace_assms t"
    and "load_auth = Some (RegAuth n) \<or> load_auth = None"
    and "{''_R29''} \<subseteq> accessible_regs s"
  ...
  shows "is_original_load_auth_cap c (run s t)"
  using assms
  by (elim CSP_or_C_read_is_original_load_auth_cap[unfolded loads_via_cap_reg_def cap_reg_is_load_auth_def]) auto
...
definition
  "VA_is_load_auth va s \<equiv>
     (if VAIsCapability va then is_load_auth_cap (VirtualAddress_base va) s else loads_via_ddc)"

lemmas VA_is_load_auth_combinators[derivable_caps_combinators] =
  Run_bindE'[where P = "\<lambda>t. VA_is_load_auth va (run s t)" for s va, simplified]
  Run_ifE[where thesis = "VA_is_load_auth va (run s t)" and t = t for s va t]
  Run_letE[where thesis = "VA_is_load_auth va (run s t)" and t = t for s va t]
  Run_case_prodE[where thesis = "VA_is_load_auth va (run s t)" and t = t for s va t]

lemma VA_is_load_auth_run_imp[derivable_caps_runI]:
  "VA_is_load_auth va s \<Longrightarrow> VA_is_load_auth va (run s t)"
  unfolding VA_is_load_auth_def
  by (auto intro: derivable_caps_runI)

lemma VAFromCapability_if_sentry_is_load_auth[derivable_capsE]:
  assumes "Run (VAFromCapability (if sentry then CapUnseal c else c)) t va"
    and "is_original_load_auth_cap c s"
  ...
  shows "VA_is_load_auth va s"
  using assms
  unfolding VA_is_load_auth_def is_load_auth_cap_def VAFromCapability_def VAIsCapability_def
  by auto

lemma VAFromCapability_is_load_auth[derivable_capsE]:
  assumes "Run (VAFromCapability c) t va"
    and "is_original_load_auth_cap c s"
  shows "VA_is_load_auth va s"
  using assms
  unfolding VA_is_load_auth_def VAFromCapability_def VAIsCapability_def
  by (auto intro: is_original_load_auth_cap_is_load_auth_cap)

lemma VAFromBits64_is_load_auth:
  assumes "Run (VAFromBits64 addr) t va"
    and "loads_via_ddc"
  shows "VA_is_load_auth va s"
  using assms
  unfolding VAFromBits64_def
  by (auto simp: VA_is_load_auth_def VAIsCapability_def)

lemma BaseReg_read_is_load_auth[derivable_capsE]:
  assumes "Run (BaseReg_read n is_prefetch) t va"
    and "load_cap_trace_assms t"
    and "load_auth = Some (BaseRegAuth n) \<or> load_auth = None"
    and "{''_R29''} \<subseteq> accessible_regs s"
    and "\<not>is_fetch"
  shows "VA_is_load_auth va (run s t)"
proof (cases is_in_c64)
  case True
  then have *: "Run (IsInC64 ()) t False \<longleftrightarrow> False" if "load_cap_trace_assms t" for t
    using that
    by (auto elim: Run_IsInC64_E)
  moreover have n: "loads_via_cap_reg n"
    using True assms(3,5)
  ...
  ultimately show ?thesis
    ...
    by -
       (elim VAFromCapability_is_load_auth[THEN VA_is_load_auth_run_imp]
             CSP_or_C_read_is_original_load_auth_cap
             derivable_capsE derivable_caps_combinators;
        (accessible_regsI | simp))
next
  case False
  then have *: "Run (IsInC64 ()) t True \<longleftrightarrow> False" if "load_cap_trace_assms t" for t
    using that
    by (auto elim: Run_IsInC64_E)
  have "loads_via_ddc"
    using False assms(3,5)
    unfolding loads_via_ddc_def ddc_is_load_auth_def
    by auto
  then show ?thesis
    using assms(1,2)
    unfolding BaseReg_read_def
    by (elim Run_bindE Run_ifE VAFromBits64_is_load_auth)
       (auto simp only: * load_cap_trace_assms_append not_not)
qed

lemmas BaseReg_read__1_is_load_auth[derivable_capsE] =
  BaseReg_read_is_load_auth[where is_prefetch = False, folded BaseReg_read__1_def]

lemma AltBaseReg_read_is_load_auth[derivable_capsE]:
  assumes "Run (AltBaseReg_read n is_prefetch) t va"
    and "load_cap_trace_assms t"
    and "load_auth = Some (AltBaseRegAuth n) \<or> load_auth = None"
    and "{''_R29''} \<subseteq> accessible_regs s"
    and "\<not>is_fetch"
  shows "VA_is_load_auth va (run s t)"
proof (cases is_in_c64)
...
lemmas AltBaseReg_read__1_is_load_auth[derivable_capsE] =
  AltBaseReg_read_is_load_auth[where is_prefetch = False, folded AltBaseReg_read__1_def]

lemma Run_CapSquashPostLoadCap_use_mem_caps:
  assumes t: "Run (CapSquashPostLoadCap c base) t c'" "load_cap_trace_assms t"
    and base: "VA_is_load_auth base s"
    and c': "CapIsTagSet c'"
...
proof (cases "VirtualAddress_vatype base")
  ...
  then show ?thesis
    ...
    by (elim Run_bindE)
       (auto simp: VA_is_load_auth_def VAIsCapability_def is_original_load_auth_cap_def
             split: if_splits dest: DDC_read_is_original_load_auth_cap)
next
  case VA_Capability
  then have "is_load_auth_cap (VirtualAddress_base base) s"
    using base
    by (auto simp: VA_is_load_auth_def VAIsCapability_def)
  from is_load_auth_cap_load_caps_permitted_iff[OF this]
  show ?thesis
    using t c' VA_Capability
    unfolding CapSquashPostLoadCap_def VAToCapability_def Let_def
    by (auto elim!: Run_bindE simp: VAIsBits64_def)
qed
...
definition enabled_branch_target :: "Capability \<Rightarrow> (Capability, register_value) axiom_state \<Rightarrow> bool" where
  "enabled_branch_target c s \<equiv> CapIsTagSet c \<longrightarrow> (\<forall>c' \<in> branch_caps c. enabled_pcc c' s)"
...
declare Run_ifE[where thesis = "enabled_branch_target (clear_lsb c) s" and a = c for c s, derivable_caps_combinators]
declare Run_letE[where thesis = "enabled_branch_target (clear_lsb c) s" and a = c for c s, derivable_caps_combinators]
declare Run_return_resultE[where P = "\<lambda>c. enabled_branch_target (clear_lsb c) s" for s, derivable_caps_combinators]

declare Run_bindE'[where P = "\<lambda>t. enabled_branch_target (clear_lsb c) (run s t)" for c s, simplified, derivable_caps_combinators]
declare Run_bindE[where thesis = "enabled_branch_target (clear_lsb c) s" and a = c for c s, derivable_caps_combinators]
declare if_split[where P = "\<lambda>c. enabled_branch_target (clear_lsb c) s" for s, THEN iffD2, derivable_capsI]

declare Run_ifE[where thesis = "enabled_branch_target (clear_lsb (CapUnseal c)) s" and a = c for c s, derivable_caps_combinators]
declare Run_letE[where thesis = "enabled_branch_target (clear_lsb (CapUnseal c)) s" and a = c for c s, derivable_caps_combinators]
declare Run_return_resultE[where P = "\<lambda>c. enabled_branch_target (clear_lsb (CapUnseal c)) s" for s, derivable_caps_combinators]

declare Run_bindE'[where P = "\<lambda>t. enabled_branch_target (clear_lsb (CapUnseal c)) (run s t)" for c s, simplified, derivable_caps_combinators]
declare Run_bindE[where thesis = "enabled_branch_target (clear_lsb (CapUnseal c)) s" and a = c for c s, derivable_caps_combinators]
declare if_split[where P = "\<lambda>c. enabled_branch_target (clear_lsb (CapUnseal c)) s" for s, THEN iffD2, derivable_capsI]

lemma enabled_branch_target_clear_lsb_CapWithTagClear[derivable_capsI]:
  "enabled_branch_target (clear_lsb (CapWithTagClear c)) s"
  by (auto simp: enabled_branch_target_def enabled_pcc_def derivable_caps_def branch_caps_128th_iff test_bit_set_gen)

lemma enabled_branch_target_clear_lsb:
  "enabled_branch_target c s \<Longrightarrow> enabled_branch_target (clear_lsb c) s"
  using tbi_enabled_cong[of "unat (c AND mask 64)" "unat (set_bit c 0 False AND mask 64)" AccType_IFETCH]
...
lemma CapIsSealed_branch_caps_empty:
  "CapIsSealed c \<Longrightarrow> branch_caps c = {}"
  by (auto simp: branch_caps_def)

lemma branch_caps_unsealed:
  "\<forall>c' \<in> branch_caps c. \<not>CapIsSealed c'"
...
lemma enabled_branch_targetI:
  ...
  using assms
  by (auto simp: enabled_branch_target_def CapIsSealed_branch_caps_empty)
...
lemma accessed_reg_caps_run_imp[derivable_caps_runI]:
  "c \<in> accessed_reg_caps s \<Longrightarrow> c \<in> accessed_reg_caps (run s t)"
...
lemma C_read_accessed_reg_caps_cases:
  assumes "Run (C_read n) t c"
...
lemma C_read_accessed_reg_caps[derivable_capsE]:
  assumes "Run (C_read n) t c"
    and "CapIsTagSet c"
    and "n = 29 \<longrightarrow> {''_R29''} \<subseteq> accessible_regs s"
  shows "c \<in> accessed_reg_caps (run s t)"
  using assms
  by (elim C_read_accessed_reg_caps_cases) auto

lemma C_read_accessed_caps_cases:
  assumes "Run (C_read n) t c"
  ...
  by (use assms in \<open>cases rule: C_read_accessed_reg_caps_cases\<close>) (auto simp: accessed_caps_def)

lemma CSP_read_accessed_reg_caps_cases:
  assumes "Run (CSP_read u) t c"
...
lemma CSP_read_accessed_caps_cases:
  assumes "Run (CSP_read u) t c"
...
proof (intro enabled_branch_targetI impI ballI)
  fix c'
  assume c: "CapIsTagSet (clear_lsb (CapUnseal c)) \<and> \<not>CapIsSealed (clear_lsb (CapUnseal c))"
  ...
  ultimately have "is_invoked_direct_sentry c' (run s t)"
    using assms c c'
    using leq_cap_trans[OF branch_caps_leq[OF c'] leq_cap_set_0th]
    by (auto simp: is_invoked_direct_sentry_def CapIsSealed_def is_sentry_def test_bit_set_gen)
...
lemma enabled_branch_target_CapWithTagClear[derivable_capsI]:
  ...
  "enabled_branch_target (clear_lsb (CapUnseal (CapWithTagClear c))) s"
  by (auto simp: enabled_branch_target_def enabled_pcc_def derivable_caps_def branch_caps_128th_iff test_bit_set_gen)
...
lemma derivable_enabled_branch_target:
  ...
  by (auto simp: enabled_branch_target_def enabled_pcc_def CapIsSealed_branch_caps_empty)
...
declare C_read_enabled_branch_target[THEN enabled_branch_target_clear_lsb, derivable_capsE]
...
lemma sealed_pair_enabled_branch_target:
...
definition
  "is_indirectly_invoked_mem_code_cap sentry type c s \<equiv>
   load_caps_permitted \<longrightarrow> is_indirectly_invoked_cap sentry type (indirect_code_cap_offset type) c s \<and> mem_branch_caps (clear_lsb c) \<subseteq> invoked_code_caps"
...
lemma CheckCapability_bounds_address:
  assumes t: "Run (CheckCapability c vaddr sz req_perms acctype) t addr" "inv_trace_assms s t"
    ...
    and "acctype' = acctype"*)
    and "valid_address acctype (unat vaddr)"
    and "CapIsTagSet c \<longrightarrow> cap_invariant c"
  shows "set (address_range (bounds_address acctype (unat vaddr)) (nat sz)) \<subseteq> get_mem_region CC c" (is ?bounds)
    and "unat vaddr + nat sz - 1 < 2 ^ 64" (is ?no_overflow)
proof -
  let ?bvaddr = "bounds_address acctype (unat vaddr)"
  have "translation_assms e" if "ev_assms s e" for s e
    by (use that in \<open>blast elim: ev_assms.elims\<close>)
  with t(2) have "translation_assms_trace t"
    by (induction t arbitrary: s) auto
  with t(1) obtain t' bvaddr
    where t': "Run (CapIsRangeInBounds c bvaddr (word_of_int sz)) t' True" "translation_assms_trace t'"
      and bvaddr: "?bvaddr = unat bvaddr"
      and addr: "addr = vaddr"
      and c: "CapIsTagSet c" "\<not>CapIsSealed c"
  ...
  then show ?no_overflow
    using bounds_address_orig_address_no_overflow[where acctype = acctype and addr = "unat vaddr" and offset = "nat sz - 1"]
    using assms c
    by (auto simp: cap_invariant_def get_mem_region_def)
qed

lemma VACheckAddress_no_overflow:
  assumes "Run (VACheckAddress base addr sz requested_perms acctype) t u" "inv_trace_assms s t"
    and sz: "sz > 0" "sz < 2^52"
    and "offset < nat sz"
    and "valid_address acctype (unat addr)"
    and "VirtualAddress_vatype base = VA_Capability \<longrightarrow> cap_invariant (VirtualAddress_base base)"
  shows "unat addr + offset < 2 ^ 64"
  using assms
  by (cases "VirtualAddress_vatype base")
     (auto simp: VACheckAddress_def VAIsBits64_def elim!: Run_bindE DDC_read_derivable[THEN derivable_caps_invariant]
           elim: inv_trace_assms_accessed_caps_invariant dest!: CheckCapability_bounds_address(2))
...
proof -
  ...
  have accessed: "sentry \<in> accessed_caps (load_caps_permitted \<and> \<not> invokes_indirect_caps) (run s t)"
    using assms(1,6,7)
    by (auto elim!: Run_bindE elim: C_read_accessed_caps_cases CSP_read_accessed_caps_cases split: if_splits)
  moreover have "cap_invariant sentry"
    using accessed inv assms(7)
    by (auto simp: accessed_caps_invariant_def)
  moreover have "CapUnseal sentry \<in> invoked_indirect_caps \<and> get_indirect_sentry_type sentry = Some type"
    using assms(1,3-5,7) assms(2)[THEN inv_invocation_trace_assms]
    by (auto elim!: Run_bindE elim!: C_read_unseal_invoked_indirect_caps_cases CSP_read_invoked_indirect_caps_cases split: if_splits)
  ultimately show ?thesis
    using assms(7-)
  ...
  then show ?thesis
    ...
    by (cases sentry_type)
       (auto elim!: Run_bindE C_read_unseal_invoked_indirect_caps_cases CSP_read_invoked_indirect_caps_cases
             simp: is_invoked_indirect_sentry_def accessed_caps_def split: if_splits)
qed
...
lemma VAFromCapability_is_invoked_indirect_sentry_for_addr[derivable_capsE]:
...
lemma is_invoked_indirect_sentry_for_addr_run[derivable_caps_runI]:
...
lemma is_indirectly_invoked_mem_pair_data_cap_run[derivable_caps_runI]:
...
proof -
  ...
  ultimately have "is_indirectly_invoked_mem_code_cap sentry sentry_type c (run s t)"
    ...
    by (cases sentry_type) auto
  then show ?thesis by blast
qed

lemma clear_lsb_member_branch_caps_imp:
  "c \<in> branch_caps c' \<Longrightarrow> clear_lsb c \<in> branch_caps c'"
  by (auto simp: branch_caps_def normalise_cursor_flags_def CapSetFlags_clear_lsb_commute)
...
lemma is_indirectly_invoked_mem_code_cap_is_single_code_cap:
  assumes "is_indirectly_invoked_mem_code_cap sentry Points_to_PCC c s"
    ...
    and "c' \<in> mem_branch_caps (clear_lsb c)"
  shows "is_indirectly_invoked_single_code_cap c' s"
  using assms
  using leq_cap_trans[OF branch_caps_leq[of c' "clear_lsb (CapUnseal c)"] leq_cap_set_0th]
  using leq_cap_trans[OF branch_caps_leq[of c' "clear_lsb c"] leq_cap_set_0th]
  using leq_cap_trans[OF branch_caps_leq[of c' "clear_perm mutable_perms (clear_lsb c)"] clear_perm_leq_cap, THEN leq_cap_trans[OF _ leq_cap_set_0th]]
  unfolding is_indirectly_invoked_single_code_cap_def is_indirectly_invoked_mem_code_cap_def
  by (fastforce simp: mem_branch_caps_def is_sentry_def CapIsSealed_def CapUnseal_clear_lsb_commute split: if_splits)
...
lemma is_indirectly_invoked_mem_code_cap_is_invoked_code_cap:
  assumes "is_indirectly_invoked_mem_code_cap sentry sentry_type c s"
    and "load_caps_permitted"
  shows "\<forall>c' \<in> mem_branch_caps (clear_lsb c). is_invoked_code_cap c' s"
  using assms(1)
  by (cases sentry_type)
...
definition
  "is_invoked_direct_mem_sentry c s \<equiv>
   c \<in> accessed_mem_caps s \<and> CapIsTagSet c \<and> is_sentry c \<and> mem_branch_caps (clear_lsb c) \<subseteq> invoked_code_caps"
...
(*lemma CapGetObjectType_clear_lsb_if_unsealed_eq[simp]:
  "CapGetObjectType (clear_lsb_if_unsealed c) = CapGetObjectType c"
  by (auto simp: clear_lsb_if_unsealed_def)*)

lemma is_invoked_direct_mem_sentry_is_invoked_direct_sentry:
  assumes "is_invoked_direct_mem_sentry c s"
    and "load_caps_permitted"
    and "\<not>invokes_indirect_caps"
  shows "\<forall>c' \<in> mem_branch_caps (clear_lsb c). is_invoked_direct_sentry c' s"
  using assms leq_cap_trans[OF branch_caps_leq[where c = "clear_lsb (CapUnseal c)"] leq_cap_set_0th]
  unfolding is_invoked_direct_sentry_def is_invoked_direct_mem_sentry_def accessed_caps_def
  by (auto simp: mem_branch_caps_def CapIsSealed_def is_sentry_def CapUnseal_clear_lsb_commute) fastforce
...
(*lemma is_invoked_direct_mem_sentry_is_invoked_direct_sentry:
  ...
  shows "is_invoked_direct_sentry c' s"
  using assms branch_caps_leq[of c' "CapUnseal c"]
  unfolding is_invoked_direct_mem_sentry_def is_invoked_direct_sentry_def accessed_caps_def
  by (auto simp: mem_branch_caps_def is_sentry_def CapIsSealed_def) fastforce*)

lemma is_invoked_mem_code_cap_is_invoked_code_cap:
  assumes "is_invoked_mem_code_cap sentry sentry_type c s"
    and "load_caps_permitted"
  shows "\<forall>c' \<in> mem_branch_caps (clear_lsb c). is_invoked_code_cap c' s"
  using assms
...
proof (intro enabled_branch_targetI impI ballI, elim conjE)
  fix c''
  assume c'': "c'' \<in> branch_caps (clear_lsb (CapUnseal c'))" and tagged: "CapIsTagSet (clear_lsb (CapUnseal c'))"
  then have "load_caps_permitted"
    using Run_CapSquashPostLoadCap_use_mem_caps[OF assms(1-3)]
    by (auto simp: test_bit_set_gen)
  have "c' = c \<and> is_sentry c \<and> is_invoked_mem_code_cap (VirtualAddress_base base) indirect_sentry_type c s"
    using tagged assms
    by (elim CapSquashPostLoadCap_cases) (auto simp: CapIsSealed_def is_sentry_def test_bit_set_gen)
  then have "is_invoked_code_cap c'' s"
    using is_invoked_mem_code_cap_is_invoked_code_cap[of "VirtualAddress_base base" indirect_sentry_type c s]
    using \<open>load_caps_permitted\<close> c'' (*unsealed lsb branch_cap*)
    by (auto simp: mem_branch_caps_def is_sentry_def CapUnseal_clear_lsb_commute)
  then show "enabled_pcc c'' (run s t)"
...
proof -
  ...
  have loaded: "mem_cap_vaddr_loaded_in_trace_if_tagged (unat addr) c t"
    using assms
    by (elim MemC_read_mem_cap_vaddr_loaded_in_trace_if_tagged) auto
  with assms(5) have loaded': "(unat addr, c) \<in> mem_cap_vaddr_loads (run s t)"
    by (auto simp: mem_cap_vaddr_loads_run_eq mem_cap_vaddr_loaded_in_trace_if_tagged_def)
  show ?thesis
  proof (cases invokes_indirect_caps)
    ...
    moreover have "load_caps_permitted \<longrightarrow> mem_branch_caps (clear_lsb c) \<subseteq> invoked_code_caps"
      ...
      by (auto simp: is_invoked_indirect_sentry_def clear_lsb_image_mem_branch_caps_eq)
    ultimately have "is_indirectly_invoked_mem_code_cap sentry sentry_type c (run s t)"
      using assms(6) loaded' is_invoked_run_mono(4)[of sentry sentry_type s t]
      unfolding is_indirectly_invoked_mem_code_cap_def is_indirectly_invoked_cap_def mem_cap_vaddr_loads_def
      by (cases sentry_type) auto
    then show ?thesis
      using True assms(4)
  ...
  next
    case False
    have "c \<in> accessed_mem_caps (run s t)"
      using loaded' \<open>CapIsTagSet c\<close>
      using imageI[where x = "(paddr, c)" and f = snd and A = "mem_cap_loads (run s t)" for paddr]
      by (auto simp: accessed_mem_caps_def mem_cap_vaddr_loads_def)
    then show ?thesis
...
(* TODO: Why doesn't this exist already? *)
lemma derivable_caps_leq_cap:
  "c \<in> derivable_caps s \<Longrightarrow> leq_cap CC c' c \<Longrightarrow> c' \<in> derivable_caps s"
  by (auto simp: derivable_caps_def leq_cap_def intro: derivable.intros)
...
lemma enabled_branch_target_CapSquashPostLoadCap:
  ...
  shows "enabled_branch_target (clear_lsb c') (run s t)"
proof (cases "invokes_indirect_caps \<and> CapIsTagSet c'")
  ...
  have "enabled_branch_target (clear_lsb c') s"
    using assms True CapIsSealed_branch_caps_empty[of "clear_lsb c"]
    by (cases rule: CapSquashPostLoadCap_cases)
       (auto simp: enabled_branch_target_def mem_branch_caps_def enabled_pcc_def CapIsSealed_def CapClearPerms_clear_lsb_commute
             dest: is_invoked_mem_code_cap_is_invoked_code_cap[OF _ \<open>load_caps_permitted\<close>] split: if_splits)
  then show ?thesis
...
next
  ...
  then have "clear_lsb c' \<in> derivable_caps s" if "CapIsTagSet c' \<and> \<not>CapIsSealed c'"
    using that
    by (auto intro: derivable_caps_leq_cap leq_cap_set_0th)
  then have "enabled_branch_target (clear_lsb c') s"
    by (auto intro: derivable_enabled_branch_target simp: test_bit_set_gen)
  then show ?thesis
    by (intro enabled_branch_target_run_imp)
qed
...
lemma MemAtomicCompareAndSwapC_from_load_auth_derivable_caps[derivable_capsE]:
  assumes "Run (MemAtomicCompareAndSwapC vaddr address expectedcap newcap ldacctype stacctype) t c" and "inv_trace_assms s t"
    and "VA_is_load_auth vaddr s" and "\<not>invokes_indirect_caps"
...
lemma enabled_branch_target_set_0th:
  assumes "enabled_branch_target c s"
...
lemma traces_enabled_BranchXToCapability[traces_enabledI]:
  assumes "enabled_branch_target (clear_lsb c) s"
...
lemma CheckCapability_load_enabled:
  assumes t: "Run (CheckCapability c vaddr sz req_perms acctype') t addr" "inv_trace_assms s t"
    ...
    and "perm_bits_included (if is_fetch then CAP_PERM_EXECUTE else CAP_PERM_LOAD) req_perms"
    and "tagged \<longrightarrow> nat sz' = 16 \<and> aligned vaddr' 16 \<and> \<not>is_fetch"
    and "CapIsTagSet c \<and> \<not>CapIsSealed c \<longrightarrow> is_load_auth_cap c s"
    and "valid_address acctype vaddr' \<and> addr = vaddr \<longrightarrow> valid_address acctype (unat vaddr)"
    and "acctype' = acctype"
  shows "load_enabled (run s t) acctype vaddr' sz' tagged"
...
next
  ...
  obtain c' where c': "c' \<in> derivable_caps s"
    and load_auth_cap: "c' \<in> original_reg_load_auth_caps"
    and load_caps: "cap_permits CAP_PERM_LOAD_CAP c \<longleftrightarrow> load_caps_permitted"
    and c: "c = c' \<or> (is_indirect_sentry CC c' \<and> c = CapUnseal c' \<and> c \<in> invoked_indirect_caps \<and> \<not>is_fetch)"
    using \<open>\<not>CapIsSealed c\<close> \<open>CapIsTagSet c \<close> \<open>CapIsTagSet c \<and> \<not>CapIsSealed c \<longrightarrow> is_load_auth_cap c s\<close>
    unfolding is_load_auth_cap_def is_original_load_auth_cap_def
    by (auto simp: cap_permits_CapUnseal_iff)
  ...
  moreover have "\<forall>is_local_cap. has_access_permission c' ?loadtype ?is_cap is_local_cap"
    ...
    by (cases is_fetch; auto simp: CC_def perms' CapIsExecutePermitted_def load_caps)
  ...
  then show "access_enabled (run s t) ?loadtype ?bvaddr' paddr' (nat sz') data ?tag"
    using derivable_caps_run_imp[OF c', where t = t] aligned tagged load_auth_cap
...
lemma VADeref_load_enabled:
  assumes "Run (VACheckAddress va vaddr sz perms acctype') t u" "inv_trace_assms s t"
    ...
    and "tagged \<longrightarrow> nat sz' = 16 \<and> aligned vaddr' 16 \<and> \<not>is_fetch"
    and "(VAIsCapability va \<longrightarrow> VAIsTaggedCap va) \<and> \<not>VAIsSealedCap va \<longrightarrow> VA_is_load_auth va s"
    and "{''PCC''} \<subseteq> accessible_regs s"
    and "valid_address acctype vaddr' \<longrightarrow> valid_address acctype (unat vaddr)"
    and "acctype' = acctype"
  shows "load_enabled (run s t) acctype vaddr' sz' tagged"
proof (cases "VirtualAddress_vatype va")
  case VA_Bits64
  then have *: "loads_via_ddc"
    using \<open>(VAIsCapability va \<longrightarrow> VAIsTaggedCap va) \<and> \<not>VAIsSealedCap va \<longrightarrow> VA_is_load_auth va s\<close>
    by (simp add: VAIsCapability_def VAIsSealedCap_def VA_is_load_auth_def)
  note ** = DDC_read_is_original_load_auth_cap[OF _ _ *, THEN is_original_load_auth_cap_is_load_auth_cap]
  show ?thesis
    using assms
    unfolding VACheckAddress_def VAIsBits64_def Let_def
    by (elim Run_bindE)
       (simp add: VA_Bits64, derivable_capsI elim: CheckCapability_load_enabled **)
next
  case VA_Capability
  then have "VAIsTaggedCap va \<and> \<not>VAIsSealedCap va \<longrightarrow> is_load_auth_cap (VirtualAddress_base va) s"
    using \<open>(VAIsCapability va \<longrightarrow> VAIsTaggedCap va) \<and> \<not>VAIsSealedCap va \<longrightarrow> VA_is_load_auth va s\<close>
    by (simp add: VA_is_load_auth_def VAIsCapability_def)
  ...
  then show ?thesis
    ...
    unfolding VACheckAddress_def VAIsBits64_def VAIsCapability_def VAIsTaggedCap_def VAIsSealedCap_def Let_def
    by (elim Run_bindE)
