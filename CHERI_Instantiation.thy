...
definition "mem_cap_loads_of_trace t \<equiv> (\<Union>e \<in> set t. mem_cap_loads_of_ev e)"

lemma read_memt_bytes_mem_cap_loads_of_trace:
  assumes "Run (read_memt_bytes BCa BCb rk addr sz) t (bytes, tag)"
    ...
    and "nat_of_bv BCa addr = Some addr'"
    and "\<not>is_translation_event ISA (E_read_memt rk addr' (nat sz) (bytes, tag))"
    and "sz = int (tag_granule ISA)"
  shows "(addr', c) \<in> mem_cap_loads_of_trace t"
  using assms
  unfolding read_memt_bytes_def maybe_fail_def mem_cap_loads_of_trace_def
  by (auto split: option.splits elim!: Traces_cases[of "Read_memt _ _ _ _"])

end

context Capability_ISA_Fixed_Translation
begin

definition mem_cap_vaddr_loads_of_trace where
  "mem_cap_vaddr_loads_of_trace t \<equiv>
     {(vaddr, c) | vaddr c paddr.
        (paddr, c) \<in> mem_cap_loads_of_trace t \<and>
        translate_address ISA vaddr Load [] = Some paddr}"

lemma mem_cap_vaddr_loads_of_traceI:
  assumes "(paddr, c) \<in> mem_cap_loads_of_trace t"
    and "translate_address ISA vaddr Load [] = Some paddr"
  shows "(vaddr, c) \<in> mem_cap_vaddr_loads_of_trace t"
  by (use assms in \<open>auto simp: mem_cap_vaddr_loads_of_trace_def\<close>)
...
lemma read_memt_bytes_accessed_mem_cap:
  ...
  by (auto simp: maybe_fail_def split: option.splits
...
lemma get_indirect_sentry_type_Some_cases:
  assumes "get_indirect_sentry_type c = Some type"
  obtains "CapGetObjectType c = CAP_SEAL_TYPE_LB" and "type = Points_to_PCC"
  | "CapGetObjectType c = CAP_SEAL_TYPE_LPB" and "type = Points_to_Pair"
  using assms
...
lemma get_mem_region_CapUnseal_eq:
...
locale Morello_Fixed_Address_Translation =
  ...
  assumes translate_correct[simp]:
    ...
    \<comment> \<open>TODO: Replace the following with an assumption that the original value of PSTATE.EL before
        it gets overwritten by @{term AArch64_TakeException} or @{term AArch64_ExceptionReturnToCapability}
        corresponds to @{term translation_el}.\<close>
...
lemma read_reg_PSTATE_translation_el:
  assumes "Run (read_reg PSTATE_ref :: ProcState M) t a"
    and "\<forall>e \<in> set t. translation_assms e"
    and "acctype \<noteq> AccType_UNPRIV"
  shows "translation_el acctype = ProcState_EL a"
  using assms
  by (intro translation_el[where t = t]) (auto simp: AArch64_AccessUsesEL_def)
...
abbreviation "instr_is_indirect_branch instr \<equiv> instr_indirect_sentry_type instr \<noteq> None"
...
abbreviation "trace_is_indirect_branch t \<equiv> trace_indirect_sentry_type t \<noteq> None"
...
text \<open>FIXME: The following definition overapproximates the set of load-authorising capabilities
  for instructions where we don't explicitly specify what register the capability is loaded from;
  in that case, we consider any capability loaded from a register as a potential authorising
  capability for now.

  Fixing that would require adding instructions to the @{type instr_ast} type and the
  @{term instr_load_auth} function as well as patching more decode clauses in the model to announce
  the instruction.

  Note that for all instructions that can perform invocation from memory, we do specify the
  register for the authorising capability, so the following definition will be exact.\<close>

definition instr_trace_load_auth_caps :: "register_value trace \<Rightarrow> Capability set" where
  "instr_trace_load_auth_caps t \<equiv>
     {c. \<exists>r.
        (\<forall>auth. trace_load_auths t = Some auth \<longrightarrow> r \<in> load_auth_reg_names (trace_is_in_c64 t) auth) \<and>
        E_read_reg r (Regval_bitvector_129_dec c) \<in> set t}"

definition fetch_trace_load_auth_caps :: "register_value trace \<Rightarrow> Capability set" where
  "fetch_trace_load_auth_caps t \<equiv> {c. E_read_reg ''PCC'' (Regval_bitvector_129_dec c) \<in> set t}"

definition trace_load_auth_caps :: "(register_value, instr) isa_trace \<Rightarrow> Capability set" where
  "trace_load_auth_caps t \<equiv>
     (case trace_kind t of
        Instr_Trace _ \<Rightarrow> instr_trace_load_auth_caps (trace t)
      | Fetch_Trace \<Rightarrow> fetch_trace_load_auth_caps (trace t))"

definition trace_has_cap_load_auth :: "register_value trace \<Rightarrow> bool" where
  "trace_has_cap_load_auth t \<equiv> (\<exists>c \<in> instr_trace_load_auth_caps t. cap_permits CAP_PERM_LOAD_CAP c)"

definition trace_uses_mem_caps :: "(register_value, instr) isa_trace \<Rightarrow> bool" where
  "trace_uses_mem_caps t \<equiv> trace_has_cap_load_auth (trace t)"
...
definition instr_of_exp :: "(register_value, 'a, 'e) monad \<Rightarrow> instr_ast option" where
  "instr_of_exp m \<equiv> if is_singleton (instrs_of_exp m) then Some (the_elem (instrs_of_exp m)) else None"

definition exp_invokes_code_cap_from_reg :: "(register_value, 'a, 'e) monad \<Rightarrow> int option" where
  "exp_invokes_code_cap_from_reg m \<equiv> Option.bind (instr_of_exp m) instr_invokes_code_cap_from_reg"

definition exp_invokes_data_cap_from_reg :: "(register_value, 'a, 'e) monad \<Rightarrow> int option" where
  "exp_invokes_data_cap_from_reg m \<equiv> Option.bind (instr_of_exp m) instr_invokes_data_cap_from_reg"

definition exp_invokes_indirect_cap_from_reg :: "(register_value, 'a, 'e) monad \<Rightarrow> int option" where
  "exp_invokes_indirect_cap_from_reg m \<equiv> Option.bind (instr_of_exp m) instr_invokes_indirect_cap_from_reg"

definition exp_invokes_indirect_caps :: "(register_value, 'a, 'e) monad \<Rightarrow> Capability set" where
  "exp_invokes_indirect_caps m \<equiv> {c. \<exists>t m'. (m, t, m') \<in> Traces \<and> c \<in> trace_invokes_indirect_sentries t}"

definition exp_load_auth :: "(register_value, 'a, 'e) monad \<Rightarrow> load_auth option" where
  "exp_load_auth m \<equiv> Option.bind (instr_of_exp m) instr_load_auth"

definition exp_indirect_sentry_type :: "(register_value, 'a, 'e) monad \<Rightarrow> indirect_sentry_type option" where
  "exp_indirect_sentry_type m \<equiv> Option.bind (instr_of_exp m) instr_indirect_sentry_type"
...
lemma instr_of_exp_write_reg_ThisInstrAbstract[simp]:
  "instr_of_exp (write_reg ThisInstrAbstract_ref instr \<bind> f) = Some instr"
  by (auto simp: instr_of_exp_def)
...
lemma no_reg_writes_to_instr_of_trace:
  ...
  using assms
  by (cases "instr_of_trace t"; fastforce simp: instr_of_trace_Some_iff no_reg_writes_to_def)

lemma no_reg_writes_to_instrs_of_exp:
  assumes "no_reg_writes_to {''__ThisInstrAbstract''} m"
  shows "instrs_of_exp m = {}"
  by (use assms in \<open>auto simp: instrs_of_exp_def no_reg_writes_to_instr_of_trace\<close>)

lemma no_reg_writes_to_instr_of_exp:
  assumes "no_reg_writes_to {''__ThisInstrAbstract''} m"
  shows "instr_of_exp m = None"
  by (use assms in \<open>auto simp: instr_of_exp_def no_reg_writes_to_instrs_of_exp is_singleton_def\<close>)

lemma exp_invokes_indirect_caps_empty_if_regs_empty[simp]:
  assumes "instr_invokes_indirect_cap_from_reg instr = None"
  shows "exp_invokes_indirect_caps (write_reg ThisInstrAbstract_ref instr \<bind> f) = {}"
  using assms
  unfolding exp_invokes_indirect_caps_def trace_invokes_indirect_sentries_def trace_invokes_indirect_cap_from_reg_def write_reg_def
  by (auto simp: bind_eq_Some_conv instr_of_trace_Some_iff register_defs elim!: Write_reg_TracesE)
...
definition "determ_instr_exp m \<equiv> (\<exists>instr f. m = write_reg ThisInstrAbstract_ref instr \<bind> f) \<or> no_reg_writes_to {''__ThisInstrAbstract''} m"

lemma write_reg_ThisInstrAbstract_Traces_instr_of_trace_cases:
  assumes "(write_reg ThisInstrAbstract_ref instr \<bind> f, t, m') \<in> Traces"
  obtains "t = []" | "instr_of_trace t = Some instr"
  by (use assms in \<open>auto simp: write_reg_def register_defs elim!: Write_reg_TracesE\<close>)

lemma determ_instr_exp_instr_of_exp_Some_cases:
  assumes "determ_instr_exp m"
    and "instr_of_exp m = Some instr"
    and "(m, t, m') \<in> Traces"
  obtains "t = []" | "instr_of_trace t = Some instr"
  by (use assms in \<open>auto simp: determ_instr_exp_def no_reg_writes_to_instr_of_exp
...
locale Morello_ISA = Morello_Fixed_Address_Translation
...
definition original_direct_mem_sentries_invoked_in_trace :: "register_value trace \<Rightarrow> Capability set" where
  "original_direct_mem_sentries_invoked_in_trace t \<equiv>
     {c. \<exists>e \<in> set t. \<exists>addr.
         ...
         (trace_indirect_sentry_type t = Some Points_to_Pair \<longrightarrow>
            (\<exists>auth \<in> instr_trace_load_auth_caps t.
               translate_address (unat (CapGetValue auth + 16)) = Some addr)) \<and>
...
definition "ISA \<equiv>
  \<lparr>isa.instr_sem = instr_sem,
   ...
   isa.trace_load_auth_caps = trace_load_auth_caps,
   ...
   isa.translate_address = \<lambda>vaddr _ _. translate_address vaddr\<rparr>"
...
lemma ISA_simps[simp]:
  ...
  "\<And>vaddr acctype t. isa.translate_address ISA vaddr acctype t = translate_address vaddr"
...
lemmas trace_invoked_cap_defs =
  instr_invokes_code_caps_def instr_invokes_data_caps_def instr_invokes_indirect_caps_def
  trace_invokes_reg_code_caps_def original_reg_code_caps_invoked_in_trace_def trace_invokes_code_cap_from_reg_def
  ...
  trace_invokes_data_cap_from_reg_def trace_indirectly_invokes_data_caps_def
  trace_invokes_direct_reg_sentries_def original_direct_reg_sentries_invoked_in_trace_def
  original_cap_pairs_invoked_in_trace_def original_reg_code_caps_read_in_trace_def
...
lemma instr_of_trace_invocation_cases:
  assumes "instr_of_trace t = Some instr"
  obtains (SealedPair) nc nd where "instr_invokes_code_cap_from_reg instr = Some nc"
    and "instr_invokes_data_cap_from_reg instr = Some nd"
    and "original_cap_pairs_invoked_in_trace t =
           {(cc, cd).
              cc \<in> trace_reads_caps_from_gpr nc t \<and>
              cd \<in> trace_reads_caps_from_gpr nd t \<and>
              invokable CC cc cd}"
    and "original_code_caps_invoked_in_trace t = {cc. \<exists>cd. (cc, cd) \<in> original_cap_pairs_invoked_in_trace t}"
    and "instr_invokes_code_caps opcode t =
           clear_lsb ` \<Union>(branch_caps ` CapUnseal ` original_code_caps_invoked_in_trace t)"
    and "instr_invokes_data_caps opcode t = {CapUnseal cd | cd. \<exists>cc. (cc, cd) \<in> original_cap_pairs_invoked_in_trace t}"
    and "instr_invokes_indirect_caps opcode t = {}"
  | (DirectRegSentry) n where "instr_invokes_code_cap_from_reg instr = Some n"
    and "instr_invokes_data_cap_from_reg instr = None"
    and "original_code_caps_invoked_in_trace t = {c. c \<in> trace_reads_caps_from_gpr n t \<and> CapIsTagSet c \<and> is_sentry c}"
    and "instr_invokes_code_caps opcode t =
           clear_lsb ` \<Union>(branch_caps ` CapUnseal ` original_code_caps_invoked_in_trace t)"
  ...
  | (DirectMemSentry)
    ...
    sentry_type where
    "instr_indirect_sentry_type instr = Some sentry_type"
    ...
    and "original_code_caps_invoked_in_trace t =
           {c. \<exists>paddr. \<exists>e \<in> set t.
                  reads_mem_cap CC e = Some (paddr, 16, c) \<and> CapIsTagSet c \<and> is_sentry c \<and>
                  (trace_indirect_sentry_type t = Some Points_to_Pair \<longrightarrow>
                     (\<exists>auth \<in> instr_trace_load_auth_caps t.
                        translate_address (unat (CapGetValue auth + 16)) = Some paddr))}"
    and "instr_invokes_code_caps opcode t =
           clear_lsb ` \<Union>(branch_caps ` CapUnseal ` original_code_caps_invoked_in_trace t)"
    and "instr_invokes_data_caps opcode t = {}"
    and "instr_invokes_indirect_caps opcode t = {}"
  | (IndirectPointsToPCC) "instr_invokes_indirect_cap_from_reg instr = Some 29"
  ...
  | (IndirectPointsToPair) n where "instr_invokes_indirect_cap_from_reg instr = Some n"
    ...
    and "instr_invokes_code_caps opcode t = clear_lsb ` \<Union>(mem_branch_caps ` original_code_caps_invoked_in_trace t)"
    and "instr_invokes_data_caps opcode t =
           \<Union>{mem_data_caps c | c. \<exists>sentry paddr vaddr. \<exists>e \<in> set t.
               reads_mem_cap CC e = Some (paddr, 16, c) \<and>
               translate_address vaddr = Some paddr \<and>
...
proof -
  ...
  proof (cases "instr_indirect_sentry_type instr" rule: indirect_sentry_type_cases)
    ...
    then show ?thesis
    proof (cases "instr_invokes_data_cap_from_reg instr" rule: option.exhaust[case_names NoDataReg DataReg])
      case NoDataReg
      then show ?thesis
      proof (cases "instr_invokes_code_cap_from_reg instr")
        case None
        then show ?thesis
          using assms NoDataReg No_Indirect_Sentry
          by (intro NoInvocation) (auto simp: trace_invoked_cap_defs)
      next
        case (Some nc)
        then show ?thesis
        proof (cases "instr_invokes_code_caps opcode t = {}")
          case True
          then show ?thesis
            using assms Some NoDataReg No_Indirect_Sentry
            by (intro NoInvocation) (auto simp: trace_invoked_cap_defs)
        next
          case False
          then show ?thesis
            using assms Some NoDataReg No_Indirect_Sentry
            by (intro DirectRegSentry[of nc])
               (auto simp: trace_invoked_cap_defs clear_lsb_image_branch_caps_eq image_UN original_code_caps_invoked_in_trace_def)
        qed
      qed
    next
      case (DataReg nd)
      then obtain nc where CodeReg: "instr_invokes_code_cap_from_reg instr = Some nc"
        by (auto elim: instr_invokes_data_cap_from_reg.elims)
      show ?thesis
      proof (cases "original_cap_pairs_invoked_in_trace t = {}")
        case True
        then show ?thesis
          using assms DataReg No_Indirect_Sentry
          by (intro NoInvocation) (auto simp: trace_invoked_cap_defs)
      next
        case False
        then show ?thesis
          using assms DataReg CodeReg No_Indirect_Sentry
          by (intro SealedPair[of nc nd])
             (auto simp: trace_invoked_cap_defs clear_lsb_image_branch_caps_eq image_UN original_code_caps_invoked_in_trace_def)
      qed
    qed
  next
    ...
    show ?thesis
    proof cases
      assume No_Indirect: "instr_invokes_indirect_caps opcode t = {}"
      show ?thesis
      proof (cases "instr_invokes_code_caps opcode t = {}")
        case True
        then show ?thesis
          using assms No_Indirect Points_to_PCC
          by (intro NoInvocation) (auto simp: trace_invoked_cap_defs elim: instr_indirect_sentry_type.elims)
      next
        case False
        show ?thesis
          using False assms No_Indirect Points_to_PCC
          by (intro DirectMemSentry[of Points_to_PCC];
              auto elim!: instr_indirect_sentry_type.elims
                   simp: trace_invoked_cap_defs if_then_Some_eq_Some_iff mem_branch_caps_sentry_eq
                         original_code_caps_invoked_in_trace_def
                         clear_lsb_image_branch_caps_eq clear_lsb_image_mem_branch_caps_eq image_UN image_Un
                         CapIsSealed_def is_sentry_def reads_mem_cap_Some_iff;
              fastforce)
      qed
    next
      assume Indirect: "instr_invokes_indirect_caps opcode t \<noteq> {}"
      then show ?thesis
        using assms Points_to_PCC
        apply (intro IndirectPointsToPCC)
        ...
        subgoal for c' x paddr vaddr rk bytes c'a imm7 Cn
          apply (erule allE[where x = rk], erule allE[where x = vaddr], erule allE[where x = paddr], erule allE[where x = bytes], erule impE, assumption, erule impE, assumption, erule allE[where x = "CapUnseal c'a"])
          apply auto
          done
        subgoal for c' x paddr vaddr rk bytes c'a imm7 Cn
  ...
  next
    ...
    proof cases
      assume No_Indirect: "instr_invokes_indirect_caps opcode t = {}"
      note reads_mem_cap_SomeE[rule del]
      show ?thesis
      proof (cases "instr_invokes_code_caps opcode t = {}")
        case True
        then show ?thesis
          using assms No_Indirect Points_to_Pair
          by (intro NoInvocation) (auto simp: trace_invoked_cap_defs elim: instr_indirect_sentry_type.elims)
      next
        case False
        show thesis
        proof (rule DirectMemSentry[OF Points_to_Pair])
          from Points_to_Pair show [simp]: "instr_invokes_code_cap_from_reg instr = None"
            and [simp]: "instr_invokes_data_cap_from_reg instr = None"
            by (auto elim: instr_indirect_sentry_type.elims)
          have direct_mem_sentry_is_sentry: "CapGetObjectType c = CAP_SEAL_TYPE_RB"
            if "c \<in> original_direct_mem_sentries_invoked_in_trace t" for c
            using that
            by (auto simp: original_direct_mem_sentries_invoked_in_trace_def is_sentry_def)
          have [simp]: "original_reg_code_caps_invoked_in_trace t = {}"
            using assms Points_to_Pair
            by (auto simp: original_reg_code_caps_invoked_in_trace_def original_direct_reg_sentries_invoked_in_trace_def
                           original_cap_pairs_invoked_in_trace_def trace_invokes_code_cap_from_reg_def)
          moreover have [simp]: "original_code_caps_indirectly_invoked_in_trace t = {}"
            using No_Indirect
            by (auto simp: original_code_caps_indirectly_invoked_in_trace_def instr_invokes_indirect_caps_def)
          moreover have "original_direct_mem_sentries_invoked_in_trace t \<noteq> {}"
            using False
            by (auto simp: instr_invokes_code_caps_def trace_indirectly_invokes_code_caps_def
                           trace_invokes_reg_code_caps_def trace_invokes_direct_mem_sentries_def)
          moreover show "instr_invokes_code_caps opcode t =
              clear_lsb ` \<Union>(branch_caps ` CapUnseal ` original_code_caps_invoked_in_trace t)"
            by (auto simp: instr_invokes_code_caps_def trace_indirectly_invokes_code_caps_def
                           trace_invokes_reg_code_caps_def trace_invokes_direct_mem_sentries_def
                           original_code_caps_invoked_in_trace_def image_iff
                           direct_mem_sentry_is_sentry[THEN mem_branch_caps_sentry_eq])
          ultimately show "original_code_caps_invoked_in_trace t =
                {c. \<exists>paddr. \<exists>e \<in> set t.
                    reads_mem_cap CC e = Some (paddr, 16, c) \<and> CapIsTagSet c \<and> is_sentry c \<and>
                    (trace_indirect_sentry_type t = Some Points_to_Pair \<longrightarrow>
                       (\<exists>auth \<in> instr_trace_load_auth_caps t.
                          translate_address (unat (CapGetValue auth + 16)) = Some paddr))}"
            and "trace_has_cap_load_auth t"
            using Points_to_Pair No_Indirect assms
            by (auto simp: original_code_caps_invoked_in_trace_def original_direct_mem_sentries_invoked_in_trace_def
                           trace_indirect_sentry_type_def instr_invokes_indirect_caps_def)
          show "\<forall>n c. instr_invokes_indirect_cap_from_reg instr = Some n \<and> c \<in> trace_reads_caps_from_gpr n t \<longrightarrow>
                        \<not>CapIsTagSet c \<or> get_indirect_sentry_type c \<noteq> Some Points_to_Pair"
            using No_Indirect Points_to_Pair assms
            by (auto simp: instr_invokes_indirect_caps_def trace_invokes_indirect_sentries_def
                           trace_indirect_sentry_type_def trace_invokes_indirect_cap_from_reg_def)
          then show "instr_invokes_data_caps opcode t = {}"
            using Points_to_Pair assms
            by (auto simp: trace_invoked_cap_defs)
          from No_Indirect show "instr_invokes_indirect_caps opcode t = {}" .
        qed
      qed
    next
      assume Indirect: "instr_invokes_indirect_caps opcode t \<noteq> {}"
      then obtain n where n: "instr_invokes_indirect_cap_from_reg instr = Some n"
        using assms
        by (auto simp: trace_invoked_cap_defs)
      then have [simp]: "instr_invokes_code_cap_from_reg instr = None"
        and [simp]: "instr_invokes_data_cap_from_reg instr = None"
        by (auto elim: instr_invokes_indirect_cap_from_reg.elims)
      show thesis
      proof (intro IndirectPointsToPair[OF n Points_to_Pair _ _ _ _ Indirect])
        show "instr_invokes_indirect_caps opcode t =
                {CapUnseal c | c. c \<in> trace_reads_caps_from_gpr n t \<and> CapIsTagSet c \<and>
                                  CapGetObjectType c = CAP_SEAL_TYPE_LPB}"
          using n Points_to_Pair assms
          by (auto simp: trace_invoked_cap_defs CapIsSealed_def)
        have [simp]: "original_reg_code_caps_invoked_in_trace t = {}"
          and [simp]: "original_direct_mem_sentries_invoked_in_trace t = {}"
          using Indirect n assms
          by (auto simp: trace_invoked_cap_defs)
        then show "original_code_caps_invoked_in_trace t =
                     {c. \<exists>sentry paddr vaddr.
                            \<exists>e\<in>set t.
                               reads_mem_cap CC e = Some (paddr, 16, c) \<and>
                               translate_address vaddr = Some paddr \<and>
                               vaddr = unat (CapGetValue sentry + 16) \<and>
                               ...
                               trace_has_cap_load_auth t}"
          using Points_to_Pair assms
          by (auto simp: original_code_caps_invoked_in_trace_def original_code_caps_indirectly_invoked_in_trace_def
                         trace_indirect_sentry_type_def instr_invokes_indirect_caps_def reads_mem_cap_Some_iff,
              fastforce+)
        show "instr_invokes_code_caps opcode t = clear_lsb ` \<Union>(mem_branch_caps ` original_code_caps_invoked_in_trace t)"
          by (auto simp: instr_invokes_code_caps_def original_code_caps_invoked_in_trace_def
                         trace_indirectly_invokes_code_caps_def trace_invokes_direct_mem_sentries_def
                         trace_invokes_reg_code_caps_def)
        show "instr_invokes_data_caps opcode t =
                \<Union> {mem_data_caps c |c.
                    \<exists>sentry paddr vaddr.
                       \<exists>e\<in>set t.
                          reads_mem_cap CC e = Some (paddr, 16, c) \<and>
                          translate_address vaddr = Some paddr \<and>
                          vaddr = unat (CapGetValue sentry) \<and>
          ...
          apply (auto simp: trace_invoked_cap_defs reads_mem_cap_Some_iff CapUnseal_get_bounds_helpers_eq get_mem_region_CapUnseal_eq CapIsSealed_def)
          subgoal for c rk paddr bytes c' c''
            apply (rule exI[where x = "mem_data_caps c''"], rule conjI, rule exI[where x = c''], rule conjI, rule refl, rule exI[where x = "CapUnseal c'"])
            by (auto simp: CapUnseal_get_bounds_helpers_eq get_mem_region_CapUnseal_eq CapIsSealed_def)
          subgoal for c c' paddr rk bytes c''
            apply (rule exI[where x = rk], rule exI[where x = paddr], rule exI[where x = bytes], rule exI[where x = "CapUnseal c''"])
            by (auto simp: CapUnseal_get_bounds_helpers_eq get_mem_region_CapUnseal_eq CapIsSealed_def)
          done
      qed
    qed
...
lemma Run_CSP_readE:
  ...
  obtains r where "E_read_reg r (Regval_bitvector_129_dec c) \<in> set t" and "r \<in> R_name 31"
  using assms
  unfolding CSP_read_def R_read_def
...
lemma read_memt_accessed_mem_cap:
  assumes "Run (read_memt BC_mword BC_mword rk addr sz) t (data, tag)"
    and "tag' \<longleftrightarrow> tag = B1" and "sz = CAPABILITY_DBYTES"
...
lemma ReadTaggedMem_lower_accessed_mem_cap:
  ...
  by (auto simp add: Bits_def nth_ucast nth_word_cat elim!: Run_bindE Run_letE Run_ifE
           dest: read_memt_accessed_mem_cap[where sz = 16 and tag' = True] split: if_splits)
...
lemma ReadTaggedMem_upper_accessed_mem_cap:
  ...
  using t sz
  unfolding ReadTaggedMem_def
  by (auto simp add: Bits_def nth_ucast nth_word_cat slice_128_cat_cap_pair split: if_splits
           elim!: Run_bindE Run_letE Run_ifE dest: read_memt_accessed_mem_cap[where sz = 16 and tag' = True])
...
| "sysreg_ev_assms s (E_read_reg r (Regval_bitvector_32_dec v)) =
     ((r = ''CSCR_EL3'' \<longrightarrow> no_system_reg_access \<or> v !! 0) \<and>
      ...
      (r = ''MDSCR_EL1'' \<longrightarrow> (\<not>v !! 15) \<and> (\<not>v !! 0)))
         \<comment> \<open>Monitor debug events (breakpoints/watchpoints) and software stepping disabled\<close>"
...
locale Morello_Load_Cap_Assms = Morello_ISA +
  fixes enabled :: "(Capability, register_value) axiom_state \<Rightarrow> register_value event \<Rightarrow> bool"
    ...
    and original_reg_load_auth_caps :: "Capability set"
    and is_fetch :: bool
begin

definition loads_via_cap_reg :: "int \<Rightarrow> bool" where
  "loads_via_cap_reg n \<equiv> (cap_reg_is_load_auth is_in_c64 n load_auth \<or> load_auth = None) \<and> \<not>is_fetch"

definition loads_via_ddc :: "bool" where
  "loads_via_ddc \<equiv> (ddc_is_load_auth is_in_c64 load_auth \<or> load_auth = None) \<and> \<not>is_fetch"

definition loads_via_pcc :: "bool" where
  "loads_via_pcc \<equiv> (pcc_is_load_auth load_auth \<or> load_auth = None \<or> is_fetch)"

fun load_cap_ev_assms :: "register_value event \<Rightarrow> bool" where
  "load_cap_ev_assms (E_read_reg r v) =
     ((r = ''PCC'' \<and> loads_via_pcc \<longrightarrow> (\<forall>c \<in> caps_of_regval v. c \<in> original_reg_load_auth_caps \<and> (load_caps_permitted \<longleftrightarrow> cap_permits CAP_PERM_LOAD_CAP c))) \<and>
      (\<forall>n c. r \<in> R_name n \<and> loads_via_cap_reg n \<and> c \<in> caps_of_regval v \<longrightarrow> c \<in> original_reg_load_auth_caps \<and> ((load_caps_permitted \<longleftrightarrow> cap_permits CAP_PERM_LOAD_CAP c))) \<and>
      (\<forall>c. r \<in> DDC_names \<and> loads_via_ddc \<and> c \<in> caps_of_regval v \<longrightarrow> c \<in> original_reg_load_auth_caps \<and> (load_caps_permitted \<longleftrightarrow> cap_permits CAP_PERM_LOAD_CAP c)) \<and>
...
sublocale Morello_Cap_Axiom_Automaton
  ...
  where use_mem_caps = "load_caps_permitted \<and> invoked_indirect_caps = {}"
    (* and translate_address = "\<lambda>addr _. translate_address addr" .. *)
  ..
...
definition is_original_load_auth_cap where
  "is_original_load_auth_cap c s \<equiv>
     (c \<in> derivable_caps s \<and> c \<in> original_reg_load_auth_caps \<and>
      (cap_permits CAP_PERM_LOAD_CAP c \<longleftrightarrow> load_caps_permitted))"

lemma DDC_read_is_original_load_auth_cap[derivable_capsE]:
  assumes t: "Run (DDC_read u) t c"
    and "load_cap_trace_assms t"
    and "loads_via_ddc"
  shows "is_original_load_auth_cap c (run s t)"
  using assms DDC_read_derivable[OF t, where s = s]
  unfolding DDC_read_def is_original_load_auth_cap_def
  by (elim Run_bindE Run_letE Run_if_ELs_cases Run_ifE Run_read_regE)
     (auto simp: register_defs load_cap_trace_assms_def DDC_names_def)

lemma read_reg_PCC_is_original_load_auth_cap[derivable_capsE]:
  assumes t: "Run (read_reg PCC_ref) t c"
    and "{''PCC''} \<subseteq> accessible_regs s"
    and "load_cap_trace_assms t"
    and "loads_via_pcc"
  shows "is_original_load_auth_cap c (run s t)"
  using assms read_reg_derivable[OF t, where s = s and c = c]
  unfolding is_original_load_auth_cap_def
  by (elim Run_read_regE) (auto simp: register_defs load_cap_trace_assms_def)
...
(*lemma BaseReg_read_VA_from_load_auth[derivable_capsE]:
  assumes t: "Run (BaseReg_read n prefetch) t va" "load_cap_trace_assms t"
    and n: "load_auth = Some (BaseRegAuth n)"
    and "\<not>is_fetch"
...
proof (cases is_in_c64)
  ...
  have "loads_via_cap_reg n"
    using True n \<open>\<not>is_fetch\<close>
    unfolding loads_via_cap_reg_def cap_reg_is_load_auth_def
    by auto
  then show ?thesis
    using t
...
next
  ...
  have *: "Run (IsInC64 ()) t True \<longleftrightarrow> False" if "load_cap_trace_assms t" for t
    ...
    by (auto elim: Run_IsInC64_E)
  have "loads_via_ddc"
    using False n \<open>\<not>is_fetch\<close>
    unfolding loads_via_ddc_def ddc_is_load_auth_def
    by auto
  then show ?thesis
    using t
    unfolding BaseReg_read_def VA_from_load_auth_def
...
lemma BaseReg_read__1_VA_from_load_auth[derivable_capsE]:
  assumes "Run (BaseReg_read__1 n) t va" "load_cap_trace_assms t"
    and "load_auth = Some (BaseRegAuth n)"
    and "\<not>is_fetch"
  shows "VA_from_load_auth va"
  using assms
...
lemma AltBaseReg_read_VA_from_load_auth[derivable_capsE]:
  assumes t: "Run (AltBaseReg_read n prefetch) t va" "load_cap_trace_assms t"
    and n: "load_auth = Some (AltBaseRegAuth n)"
    and "\<not>is_fetch"
  shows "VA_from_load_auth va"
proof (cases is_in_c64)
  case True
...
lemma AltBaseReg_read__1_VA_from_load_auth[derivable_capsE]:
  assumes "Run (AltBaseReg_read__1 n) t va" "load_cap_trace_assms t"
    and "load_auth = Some (AltBaseRegAuth n)"
    and "\<not>is_fetch"
  shows "VA_from_load_auth va"
  using assms
...
lemma RegAuth_loads_via_cap_regI[derivable_capsI, intro, simp]:
  "load_auth = Some (RegAuth n) \<Longrightarrow> \<not>is_fetch \<Longrightarrow> loads_via_cap_reg n"
  unfolding loads_via_cap_reg_def cap_reg_is_load_auth_def
  by auto
...
lemma loads_via_pccI[derivable_capsI, intro, simp]:
  assumes "load_auth = Some PCCAuth \<or> load_auth = None \<or> is_fetch"
  shows "loads_via_pcc"
  using assms
  unfolding loads_via_pcc_def pcc_is_load_auth_def
  by auto

definition load_instr_exp_assms :: "(register_value, 'a, 'e) monad \<Rightarrow> bool" where
  "load_instr_exp_assms m \<equiv> load_auth = exp_load_auth m"

lemma load_instr_exp_assms_write_ThisInstrAbstract_iff:
  "load_instr_exp_assms (write_reg ThisInstrAbstract_ref instr \<bind> f)
   \<longleftrightarrow> load_auth = instr_load_auth instr"
  by (auto simp: load_instr_exp_assms_def exp_load_auth_def)
...
locale Morello_Cap_Invocation_Assms = Morello_ISA +
  fixes enabled :: "(Capability, register_value) axiom_state \<Rightarrow> register_value event \<Rightarrow> bool"
    and use_mem_caps :: "bool"
    and instr_opt :: "instr_ast option"
...
abbreviation "invoked_data_reg \<equiv> Option.bind instr_opt instr_invokes_data_cap_from_reg"
abbreviation "invoked_indirect_reg \<equiv> Option.bind instr_opt instr_invokes_indirect_cap_from_reg"
abbreviation "indirect_sentry_type \<equiv> Option.bind instr_opt instr_indirect_sentry_type"
abbreviation "is_indirect_branch \<equiv> indirect_sentry_type \<noteq> None"
...
| "invocation_ev_assms (E_read_memt rk paddr sz (bytes, tag)) \<longleftrightarrow>
    ...
    (is_indirect_branch \<and> invoked_indirect_caps = {} \<and> use_mem_caps \<longrightarrow>
       (\<forall>c. cap_of_mem_bytes bytes tag = Some c \<and> sz = nat CAPABILITY_DBYTES \<and> CapIsTagSet c \<and> is_sentry c \<and>
              (indirect_sentry_type = Some Points_to_Pair \<longrightarrow>
                 (\<exists>auth \<in> original_reg_load_auth_caps. translate_address (unat (CapGetValue auth + 16)) = Some paddr))
            \<longrightarrow>
...
(*lemma
  assumes "instr_of_exp m = Some instr"
    and "(m, t, m') \<in> Traces"
  obtains "t = []" | t' where "t = E_write_reg ''__ThisInstrAbstract'' (Regval_instr_ast instr) # t'"
proof -
  have "instr_of_trace t = Some instr" if "t \<noteq> []"
    using assms that
    apply (cases t)
     apply (auto simp: instr_of_exp_def instrs_of_exp_def is_singleton_def instr_of_trace_Some_iff split: if_splits)
  oops*)

(* TODO: Move *)
lemma instr_of_trace_Some_simps[simp]:
  fixes instr :: instr_ast
  assumes "instr_of_trace t = Some instr"
  shows "trace_invokes_code_cap_from_reg t = instr_invokes_code_cap_from_reg instr"
    and "trace_invokes_data_cap_from_reg t = instr_invokes_data_cap_from_reg instr"
    and "trace_invokes_indirect_cap_from_reg t = instr_invokes_indirect_cap_from_reg instr"
    and "trace_indirect_sentry_type t = instr_indirect_sentry_type instr"
  unfolding trace_invokes_code_cap_from_reg_def trace_invokes_data_cap_from_reg_def
...
(* TODO: Move *)
lemma determ_instr_of_exp_instr_of_trace:
  fixes instr :: instr_ast
  assumes "determ_instr_exp m"
    and "instr_of_exp m = Some instr"
    and "(m, t, m') \<in> Traces"
    and "t \<noteq> []"
  shows "instr_of_trace t = Some instr"
  by (use assms in \<open>blast elim: determ_instr_exp_instr_of_exp_Some_cases\<close>)

(* TODO: Find another way to rewrite reads_mem_cap below *)
...
proof -
  have "invocation_ev_assms e" for e
    by (induction e rule: invocation_ev_assms.induct; unfold invocation_ev_assms.simps)
       (use assms in auto)
  then show ?thesis
...
sublocale Morello_Cap_Axiom_Automaton
  where use_mem_caps = "use_mem_caps \<and> invoked_indirect_caps = {}"
  ..
  (* where translate_address = "\<lambda>addr _. translate_address addr" .. *)

definition invocation_instr_exp_assms :: "(register_value, 'a, 'e) monad \<Rightarrow> bool" where
  "invocation_instr_exp_assms m \<equiv>
     instr_opt = instr_of_exp m \<and>
     (exp_invokes_indirect_cap_from_reg m = None \<longrightarrow> invoked_indirect_caps = {})"
...
lemma invocation_instr_exp_assms_write_ThisInstrAbstract_iff:
  "invocation_instr_exp_assms (write_reg ThisInstrAbstract_ref instr \<bind> f)
   \<longleftrightarrow> (instr_opt = Some instr
     \<and> (instr_invokes_indirect_cap_from_reg instr = None \<longrightarrow> invoked_indirect_caps = {}))"
  unfolding invocation_instr_exp_assms_def exp_invokes_code_cap_from_reg_def exp_invokes_data_cap_from_reg_def
  unfolding exp_invokes_indirect_cap_from_reg_def exp_indirect_sentry_type_def
  by auto
...
lemma C_read_original_reg_load_auth_caps[derivable_capsE]:
  assumes "Run (C_read n) t c" and "invocation_trace_assms t"
    and "Option.bind instr_opt instr_load_auth = Some (RegAuth n)"
    and "CapIsTagSet c"
  shows "c \<in> original_reg_load_auth_caps"
proof -
  obtain r where "invocation_ev_assms (E_read_reg r (Regval_bitvector_129_dec c))" and "r \<in> R_name n"
    using assms(1,2,4)
    by (elim Run_C_readE) (auto simp: CapNull_def invocation_trace_assms_def)
...
lemma CapGetObjectType_get_indirect_sentry_type[unfolded special_otype_defs, simp]:
  "CapGetObjectType c = CAP_SEAL_TYPE_LB \<Longrightarrow> get_indirect_sentry_type c = Some Points_to_PCC"
...
lemma CSP_read_original_reg_load_auth_caps[derivable_capsE]:
  assumes "Run (CSP_read u) t c" and "invocation_trace_assms t"
    and "Option.bind instr_opt instr_load_auth = Some (RegAuth 31)"
  shows "c \<in> original_reg_load_auth_caps"
proof -
  obtain r where "invocation_ev_assms (E_read_reg r (Regval_bitvector_129_dec c))" and "r \<in> R_name 31"
    using assms(1,2)
...
lemma CSP_or_C_read_original_reg_load_auth_caps[derivable_capsE]:
  assumes "Run (if n = 31 then CheckSPAlignment u \<then> CSP_read u' else C_read n) t c" and "invocation_trace_assms t"
    and "Option.bind instr_opt instr_load_auth = Some (RegAuth n)"
    and "CapIsTagSet c"
  shows "c \<in> original_reg_load_auth_caps"
  using assms
  by (auto elim!: CSP_read_original_reg_load_auth_caps C_read_original_reg_load_auth_caps Run_bindE split: if_splits)
...
lemma mem_cap_loads_of_trace_append[simp]:
  "mem_cap_loads_of_trace (t1 @ t2) = mem_cap_loads_of_trace t1 \<union> mem_cap_loads_of_trace t2"
  unfolding mem_cap_loads_of_trace_def
  by auto

lemma mem_cap_loads_run_eq:
  "mem_cap_loads (run s t) = mem_cap_loads_of_trace t \<union> mem_cap_loads s"
  by (induction t arbitrary: s) (auto simp: mem_cap_loads_of_trace_def)

definition (in Capability_ISA_Fixed_Translation)
  "mem_cap_vaddr_loads s \<equiv>
     {(vaddr, c). \<exists>paddr.
        (paddr, c) \<in> mem_cap_loads s \<and> isa.translate_address ISA vaddr Load [] = Some paddr}"

(*lemma member_mem_cap_vaddr_loads_iff:
  "(vaddr, c) \<in> mem_cap_vaddr_loads s \<longleftrightarrow>
   (\<exists>paddr. (paddr, c) \<in> mem_cap_loads s \<and> isa.translate_address ISA vaddr Load [] = Some paddr)"
  by (auto simp: mem_cap_vaddr_loads_def)*)

definition (in Capability_ISA_Fixed_Translation)
  "mem_cap_loaded_in_trace_if_tagged paddr c t \<equiv>
   is_tagged_method CC c \<longrightarrow> (paddr, c) \<in> mem_cap_loads_of_trace t"

definition (in Capability_ISA_Fixed_Translation)
  "mem_cap_vaddr_loaded_in_trace_if_tagged vaddr c t \<equiv>
   is_tagged_method CC c \<longrightarrow> (vaddr, c) \<in> mem_cap_vaddr_loads_of_trace t"

lemma (in Capability_ISA_Fixed_Translation) mem_cap_vaddr_loaded_in_trace_if_tagged_untagged[simp]:
  assumes "\<not>is_tagged_method CC c"
  shows "mem_cap_vaddr_loaded_in_trace_if_tagged vaddr c t"
  by (use assms in \<open>auto simp: mem_cap_vaddr_loaded_in_trace_if_tagged_def\<close>)

lemma mem_cap_vaddr_loaded_in_trace_if_tagged_append[simp]:
  "mem_cap_vaddr_loaded_in_trace_if_tagged vaddr c (t1 @ t2) \<longleftrightarrow>
   mem_cap_vaddr_loaded_in_trace_if_tagged vaddr c t1 \<or> mem_cap_vaddr_loaded_in_trace_if_tagged vaddr c  t2"
  by (auto simp: mem_cap_vaddr_loaded_in_trace_if_tagged_def mem_cap_vaddr_loads_of_trace_def)

lemma mem_cap_vaddr_loads_run_eq:
  "mem_cap_vaddr_loads (run s t) = mem_cap_vaddr_loads_of_trace t \<union> mem_cap_vaddr_loads s"
  by (auto simp: mem_cap_vaddr_loads_def mem_cap_vaddr_loads_of_trace_def mem_cap_loads_run_eq)

lemma (in Capability_ISA) mem_cap_loads_of_traceE:
  assumes "(paddr, c) \<in> mem_cap_loads_of_trace t"
  obtains e where "e \<in> set t" and "(paddr, c) \<in> mem_cap_loads_of_ev e"
  by (use assms in \<open>auto simp: mem_cap_loads_of_trace_def\<close>)

lemma (in Capability_ISA) mem_cap_loads_of_evE:
  ...
  by (cases e) (use assms in \<open>auto split: option.splits if_splits\<close>)

lemma invocation_trace_assmsD:
  assumes "invocation_trace_assms t" and "e \<in> set t"
  shows "invocation_ev_assms e"
  by (use assms in \<open>auto simp: invocation_trace_assms_def\<close>)

lemma mem_cap_vaddr_loaded_in_trace_if_tagged_from_paddr:
  assumes "mem_cap_loaded_in_trace_if_tagged paddr c t"
    and "isa.translate_address ISA vaddr Load [] = Some paddr"
  shows "mem_cap_vaddr_loaded_in_trace_if_tagged vaddr c t"
  using assms
  unfolding mem_cap_loaded_in_trace_if_tagged_def mem_cap_vaddr_loaded_in_trace_if_tagged_def
  by (auto simp: mem_cap_vaddr_loads_of_trace_def)

lemma (in Capability_ISA_Fixed_Translation) read_memt_bytes_mem_cap_loaded_in_trace_if_tagged:
  assumes "Run (read_memt_bytes BCa BCb rk addr sz) t (bytes, tag)"
    and "cap_of_mem_bytes_method CC bytes tag = Some c"
    and "nat_of_bv BCa addr = Some addr'"
    and "\<not>is_translation_event ISA (E_read_memt rk addr' (nat sz) (bytes, tag))"
    and "sz = int (tag_granule ISA)"
  shows "mem_cap_loaded_in_trace_if_tagged addr' c t"
  using read_memt_bytes_mem_cap_loads_of_trace[OF assms(1,2) _ assms(3,4,5)]
  by (auto simp: mem_cap_loaded_in_trace_if_tagged_def)

lemma read_memt_mem_cap_loaded_in_trace_if_tagged:
  assumes "Run (read_memt BC_mword BC_mword rk addr sz) t (data, tag)"
    and "tag' \<longleftrightarrow> tag = B1" and "sz = int (tag_granule ISA)"
  shows "mem_cap_loaded_in_trace_if_tagged (unat addr) (Capability_of_tag_word tag' data) t"
proof cases
  assume "tag = B1"
  then show ?thesis
    using assms no_cap_load_translation_events
    unfolding read_memt_def maybe_fail_def Capability_of_tag_word_def
    by (auto simp: cap_of_mem_bytes_def split: option.splits
             elim!: Run_bindE read_memt_bytes_mem_cap_loaded_in_trace_if_tagged)
next
  assume "tag \<noteq> B1"
  with assms(2) show ?thesis
    by (auto simp: mem_cap_loaded_in_trace_if_tagged_def)
...
abbreviation "AddressDescriptor_paddr accdesc \<equiv> FullAddress_address (AddressDescriptor_paddress accdesc)"

(*lemma List_fold_un_Un:
  "List.fold (\<union>) xs ys = \<Union>(set xs) \<union> ys"
  by (induction xs arbitrary: ys) auto

lemma
  "mem_cap_loads_of_trace t = (\<Union>e \<in> set t. mem_cap_loads_of_ev e)"
  unfolding mem_cap_loads_of_trace_def List_fold_un_Un
  by auto*)

lemma ReadMemTagged_mem_cap_loaded_in_trace_if_tagged:
  assumes "Run (ReadTaggedMem desc 16 accdesc) t (tag, data)"
  shows "mem_cap_loaded_in_trace_if_tagged (unat (AddressDescriptor_paddr desc)) (Capability_of_tag_word (tag !! 0) data) t"
  using assms
  by (auto simp: ReadTaggedMem_def Bits_def elim!: Run_bindE split: if_splits
           dest: read_memt_mem_cap_loaded_in_trace_if_tagged[where tag' = "tag !! 0"])

lemmas ReadMemTagged_mem_cap_vaddr_loaded_in_trace_if_tagged =
  ReadMemTagged_mem_cap_loaded_in_trace_if_tagged[THEN mem_cap_vaddr_loaded_in_trace_if_tagged_from_paddr]

lemma AArch64_TaggedMemSingle_mem_cap_vaddr_loaded_in_trace_if_tagged:
  assumes t: "Run (AArch64_TaggedMemSingle vaddr sz acctype wasaligned) t a"
    and sz: "sz = 16"
    and translation: "translation_assms_trace t"
    and abort: "\<forall>vaddr fault. exp_fails (AArch64_Abort vaddr fault)"
  shows "mem_cap_vaddr_loaded_in_trace_if_tagged (unat vaddr) (Capability_of_tag_word (vec_of_bits [access_vec_dec (fst a) 0] !! 0) (slice (snd a :: 128 word) 0 128)) t"
  using assms
  unfolding AArch64_TaggedMemSingle_def
  by (auto simp: test_bit_of_bl elim!: Run_bindE Run_ifE
           dest: ReadMemTagged_mem_cap_vaddr_loaded_in_trace_if_tagged[where vaddr = "unat vaddr"])

lemma MemC_read_mem_cap_vaddr_loaded_in_trace_if_tagged:
  assumes "Run (MemC_read vaddr acctype) t c"
    and translation: "translation_assms_trace t"
    and abort: "\<forall>vaddr fault. exp_fails (AArch64_Abort vaddr fault)"
  shows "mem_cap_vaddr_loaded_in_trace_if_tagged (unat vaddr) c t"
  using assms
  unfolding MemC_read_def
  by (auto simp: test_bit_of_bl CapabilityFromData_def elim!: Run_bindE
           dest: AArch64_TaggedMemSingle_mem_cap_vaddr_loaded_in_trace_if_tagged)

(*lemma (in Capability_ISA) member_fold_Un_map_cases:
  assumes "y \<in> List.fold (\<union>) (map f xs) ys"
  obtains x where "x \<in> set xs" and "y \<in> f x" | "y \<in> ys"
proof (use assms in \<open>induction xs arbitrary: ys\<close>)
  case (Cons x xs)
  show ?case
    using Cons.IH[of "f x \<union> ys"] Cons.prems
    by auto
qed auto*)
...
lemma mem_cap_vaddr_loaded_in_trace_if_tagged_invoked_code_cap:
  assumes "mem_cap_vaddr_loaded_in_trace_if_tagged vaddr c t"
    and "invocation_trace_assms t"
...
lemma mem_cap_vaddr_loaded_in_trace_if_tagged_invoked_direct_mem_sentry:
  assumes "mem_cap_vaddr_loaded_in_trace_if_tagged vaddr c t"
    and "invocation_trace_assms t"
    and "CapIsTagSet c"
    and "is_sentry c"
    and "is_indirect_branch"
    and "indirect_sentry_type = Some Points_to_Pair \<longrightarrow>
           (\<exists>auth \<in> original_reg_load_auth_caps. vaddr = unat (CapGetValue auth + 16))"
...
abbreviation "load_auth \<equiv> Option.bind instr_opt instr_load_auth"

sublocale Morello_Load_Cap_Assms where load_auth = load_auth ..
sublocale Morello_Cap_Invocation_Assms
 (* where use_mem_caps = "invoked_indirect_caps = {} \<and> load_caps_permitted" .. *)
  where use_mem_caps = load_caps_permitted ..
...
lemma (in Morello_ISA) branch_caps_128th_iff:
...
lemma (in Morello_ISA) BranchAddr_in_branch_caps:
  assumes "Run (BranchAddr c el) t c'"
    and "CapIsTagSet c'"
  ...
  by (cases "CapIsSealed c")
     (auto elim!: Run_bindE Run_letE Run_ifE Run_and_boolM_E Run_or_boolM_E dest!: tbi_enabled
           simp: CapSetFlags_mask_56_normalise_cursor_flags CapSetFlags_SignExtend_normalise_cursor_flags)

lemma (in Morello_ISA) BranchAddr_branch_caps_tagged_unsealed:
  assumes "Run (BranchAddr c el) t c'" and "CapIsTagSet c'"
    and "translation_el AccType_IFETCH = el"
    and "translation_assms_trace t"
...
locale Morello_Axiom_Automaton =
  ...
  Cap_Axiom_Assm_Automaton where CC = CC and ISA = ISA and initial_caps = UNKNOWN_caps
    ...
    and use_mem_caps = "load_caps_permitted \<and> invoked_indirect_caps = {}"
begin
...
lemma invocation_ev_assmsI[intro, simp]:
  ...
  by (elim ev_assms.elims; (blast | simp))
...
lemma unknown_ev_assmsI[intro, simp]:
  ...
  by (elim ev_assms.elims; simp only: unknown_ev_assms.simps)
...
(* The __DecodeA64 function generated by asl_to_sail takes the PC as its first
   argument, for compatibility with some Arm-internal tooling.  However, it does
   not actually use the PC argument. *)
lemma DecodeA64_ignore_pc: "DecodeA64 pc opcode = DecodeA64 0 opcode"
  by (unfold DecodeA64_def, rule refl)

lemma instr_exp_assms_instr_opt_None:
  "instr_exp_assms m \<Longrightarrow> no_reg_writes_to {''__ThisInstrAbstract''} m \<Longrightarrow> instr_opt = None"
  by (auto simp: instr_exp_assms_def invocation_instr_exp_assms_def intro: no_reg_writes_to_instr_of_exp)

lemma instr_exp_assms_load_auth_None:
  "instr_exp_assms m \<Longrightarrow> no_reg_writes_to {''__ThisInstrAbstract''} m \<Longrightarrow> load_auth = None"
  by (auto dest: instr_exp_assms_instr_opt_None)
...
sublocale Write_Cap_Automaton
  where CC = CC and ISA = ISA and initial_caps = UNKNOWN_caps
    (* and use_mem_caps = "invoked_indirect_caps = {} \<and> load_caps_permitted" *)
...
locale Morello_Mem_Axiom_Automaton = Morello_ISA +
  fixes ex_traces :: bool
    and instr_opt :: "instr_ast option"
    and invoked_code_caps :: "Capability set"
    and invoked_data_caps :: "Capability set"
...
sublocale Mem_Automaton
  where CC = CC and ISA = ISA and initial_caps = UNKNOWN_caps
    and use_mem_caps = load_caps_permitted
    and load_auth_caps = original_reg_load_auth_caps
  ..
...
locale Morello_Trace_Axiom_Automaton = Morello_ISA +
  ...
  where ex_traces = "isa.trace_raises_ex ISA t"
    ...
    and original_reg_load_auth_caps = "isa.trace_load_auth_caps ISA t"
...
locale Morello_Trace_Write_Cap_Automaton = Morello_ISA +
...
locale Morello_Trace_Mem_Automaton = Morello_ISA +
...
sublocale Morello_Instr_Trace_Axiom_Automaton \<subseteq> Morello_Instr_Axiom_Automaton
  where ex_traces = "isa.trace_raises_ex ISA (instr_trace instr t)"
    ...
    and original_reg_load_auth_caps = "isa.trace_load_auth_caps ISA (instr_trace instr t)"
...
sublocale Morello_Fetch_Trace_Axiom_Automaton \<subseteq> Morello_Fetch_Axiom_Automaton
  where ex_traces = "isa.trace_raises_ex ISA (fetch_trace t :: (register_value, instr) isa_trace)"
    ...
    and original_reg_load_auth_caps = "isa.trace_load_auth_caps ISA (fetch_trace t :: (register_value, instr) isa_trace)"
